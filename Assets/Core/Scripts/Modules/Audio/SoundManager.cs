...
public class SoundManager: MonoBehaviour
{
    ...
    public void PlayFX(string fxName, Vector3? position = null, float pitchRange = 0.2f)
    {
        if (TryGetAudioSource(fxName, FXSounds, out var fx, position))
        {
            fx.pitch = 1 - Random.Range(-pitchRange, pitchRange);
    ...
    public void PlayUISound(string uiSoundName, Vector3? position = null, float pitchRange = 0.2f)
    {
        if (TryGetAudioSource(uiSoundName, UISounds, out var uiSound, position))
        {
            uiSound.pitch = 1 - Random.Range(-pitchRange, pitchRange);
